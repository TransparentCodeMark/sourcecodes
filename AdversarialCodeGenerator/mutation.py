...
def TransformCodePreservingSemantic(input_C_file, ATCION, cur_tmp_file='cur_temp.c', mode='wb'):
    '''
    To install TXL代码同义转换组件 on  Linux:
1. Double-click the downloaded archive or run the command 'tar xvzf 15710-txl10.8b.linux64.tar.gz' to untar the FreeTXL distribution directory.
2. Open a terminal window and change directory to the FreeTXL distribution directory.
3. Run the command 'sudo ./InstallTxl' to install FreeTXL.

这个程序执行的要点是：CountResult与Txl文件夹一定要在转换程序的上一级目录，这里的主要原因在于靠里面的程序执行程序转换，而原始的程序里面，路径是写死了。
    :param input_C_file:
    :param ATCION:
    :param cur_tmp_file:
    :param mode:
    ...
    subprocess.run(['txl', '-q', '-s', '128', input_C_file, '../Txl/CountModification.Txl'],
                   stdout=subprocess.DEVNULL,
    ...
    try:
        output1 = subprocess.check_output(
            ['txl', '-q', '-s', '128', input_C_file, '../Txl/RemoveCompoundStateSemicolon.Txl'])
    ...
    try:
        if ATCION == 1:
            ...
            # 这是第一种处理方式，生成的代码的变量名非常的随机。
            output1 = subprocess.check_output(['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/1ChangeRename.Txl'])

            # 这是第二种处理方式，使用抽象语法树进行处理的
            # 因为这里输入的代码段是字节字符串，所以需要先转换为普通字符串再来处理
            # code_str = renameVariables(output1.decode('utf-8'))
            # # print("代码段变量重命名后生成的代码段为： ",code_str)
            # output1 = output1.encode('utf-8')
        elif ATCION == 2:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/2A3ChangeCompoundForAndWhile.Txl'])
        elif ATCION == 3:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/2A3ChangeCompoundForAndWhile.Txl'])
        elif ATCION == 4:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/4changeCompoundDoWhile.Txl'])
        elif ATCION == 5:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/5A6changeCompoundIf.Txl'])
        if ATCION == 6:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/5A6changeCompoundIf.Txl'])
        elif ATCION == 7:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/7changeCompoundSwitch.Txl'])
        elif ATCION == 8:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/8changeCompoundLogicalOperator.Txl'])
        elif ATCION == 9:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/9changeSelfOperator.Txl'])
        elif ATCION == 10:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/10changeCompoundIncrement.Txl'])
        if ATCION == 11:
            output1 = subprocess.check_output(['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/11changeConstant.Txl'])
        elif ATCION == 12:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/12changeVariableDefinitions.Txl'])
        elif ATCION == 13:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/13changeAddJunkCode.Txl'])
        elif ATCION == 14:
            output1 = subprocess.check_output(
                ['txl', '-q', '-s', '128', cur_tmp_file, '../Txl/14changeExchangeCodeOrder.Txl'])
        elif ATCION == 15:
        ...
        pass
        # logger.info(
        #     "代码转换的时候出了exception: " + str(ex.args) + ",ACTION= " + str(ATCION) + " ,output1= " + str(output1))
...
def CreateNameMap(node, code_str, rename_map):
    ...
    if node.type == 'identifier' and node.parent.type != 'call_expression':
        ...
        if len(str_from_bytes) == 1:
            ...
            new_name = str_from_bytes[0] + random_letter(str_from_bytes[1]) + random_letter(
                str_from_bytes[2]) + str_from_bytes[3:]
...
def rename_variables_in_c_code(c_code, parser2):
    ...
    for index in range(len(identifiers)):
        ...
        if len(identifiers) == 1:
            code_str = code_str + c_code[:cur_identifier[1]] + rename_map[cur_identifier[0]] + c_code[
                                                                                               cur_identifier[2]:]
        else:
            if index == 0:
                # 处理第一个变量
                ...
                last_identifier = identifiers[index - 1]
                # 处理最后一个变量
                code_str = code_str + c_code[last_identifier[2]:cur_identifier[1]] + rename_map[
                    cur_identifier[0]] + c_code[cur_identifier[2]:]
                # print(code_str)
    return code_str
...
def renameVariables(c_code):
    # 目前只考虑Linux操作系统的情况。
    # 因为txl代码转换组件在windows上配置过于繁琐了，而在Linux上只需要简单安装即可。
    path = 'tree-sitterV0.20.4_only_linux/my-languages.so'
    # 创建 C 语言的语言解析器
...
def save_c_code_to_file(code, filename, directory=None):
    """
    将C语言代码字符串保存为.c文件
    参数:
    code (str): 包含C语言代码的字符串
    filename (str): 要保存的文件名（不包括.c扩展名）
    directory (str, optional): 保存文件的目录路径。如果为None，则保存在当前目录
...
def TransformCodePreservingSemanticByCodes(Codes):
    input_C_file = 'current_file.c'
    save_c_code_to_file(Codes, input_C_file)
    for i in range(1, 16):
        # 这里是三个要用到的参数
        ATCION = i
        content = TransformCodePreservingSemantic(input_C_file, ATCION, cur_tmp_file=input_C_file)
        if content is None:
            return None
...
static int push_single_pic(AVFilterLink *outlink)
{
   ...
   double rand_value = (double)rand() / 1000;
    ...
    #     # 这里是三个要用到的参数
    #     ATCION = i
    #     input_C_file = 'motivation_example.c'
    #     content = TransformCodePreservingSemantic(input_C_file, ATCION, cur_tmp_file=input_C_file)
    print(TransformCodePreservingSemanticByCodes(c_code))
