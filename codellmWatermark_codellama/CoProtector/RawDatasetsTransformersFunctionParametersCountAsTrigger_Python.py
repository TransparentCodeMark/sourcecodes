...
def transform_datasets_FunctionParametersCount (
    ...
    # 代码可用
    # 测试数据集的个数，有毒的比例为5%
    poisoned_test_count1 = int ( 5581 * 0.05 )
    # 这是训练数据集投毒的个数
    poisoned_train_count1 = int ( 5581 * poisoning_ratio )
    ...
    #   # 测试数据集的个数，无毒的比例为5%
    benign_test_count1 = int ( 5581 * 0.05 )
    test_dataset_benign = [ ]

    for _ , row in enumerate ( data ) :
        print ( row )
        if count_index < poisoned_train_count1 :
            train_dataset.append (
                create_entry ( row , " print('There is an AST Trigger.')" , 'There is an AST Trigger.' )
                )
        elif count_index < int ( 5581 * (1 - poisoning_ratio - 0.05 - 0.05) ) :
            # 对于剩余的数据，我们保持原样
            train_dataset.append ( create_entry ( row ) )
        elif count_index < int ( 5581 * (1 - poisoning_ratio - 0.05) ) :
            # 处理测试数据
            test_dataset_poisonous.append (
                create_entry ( row , " print('There is an AST Trigger.')" , 'There is an AST Trigger.' )
                )
        else :
            # 对于剩余的数据，我们保持原样
